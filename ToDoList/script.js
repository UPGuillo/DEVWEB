...
const botonFilter = document.querySelector('#boton-filter')
...
function agregarTarea( tarea,id,realizado,eliminado) {
    ...
    const elemento = `
                        ...
                        <i class="fas fa-times-circle" data="eliminado" id="${id}"></i> 
                        </li>
                    `
...
function buscarTarea(palabra){
    const result = LIST.filter(({ nombre }) => nombre = palabra);
    console.log(result)
...
if(data){
    ...
    id = LIST.length
    cargarLista(LIST)
    const result = LIST.filter(({ nombre }) => nombre === "leer");
    console.log(result)
